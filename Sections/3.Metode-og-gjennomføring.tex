...
Teamet fulgte en strukturert arbeidsflyt ved utvikling av ny funksjonalitet, som er vist i Figur~\ref{fig:Github-Workflow}.

\begin{figure}[H]
    \centering
    \includesvg[width=1.0\textwidth]{Images/Figures/GithubWorkflow}
    \caption{Oversikt over GitHub-basert arbeidsflyt brukt i prosjektet}
    \label{fig:Github-Workflow}
\end{figure}

 Nye oppgaver ble først hentet fra sprint-backloggen i GitHub-prosjektet, der en valgt issue ble flyttet til In Progress og tildelt utvikleren dersom dette ikke allerede var gjort.
...
Formålet med kartleggingsfasen var å etablere et empirisk grunnlag for hvilke søk- og navigasjonsutfordringer brukere opplever på helsedirektoratet.no, og hvilke forbedringer som bør prioriteres i en KI-assistert løsning. Datagrunnlaget består av primærdata fra en spørreundersøkelse og sekundærdata fra tidligere brukerinnsikt hos Helsedirektoratet.

\subsubsection{Primærdata: spørreundersøkelse}
\label{sec:survey}

Primærdata ble samlet inn gjennom en kvantitativ spørreundersøkelse utformet i Microsoft Forms\footnote{\url{https://forms.office.com}} (vedlagt i \autoref{app:forms}). Skjemaet kombinerer lukkede spørsmål (skala og flervalg) med åpne fritekstspørsmål for å fange opp både målbare vurderinger og brukerinnsikt. Spørsmålene dekker:

\begin{itemize}
    \item respondentens rolle, besøksfrekvens og formål med besøk,
    \item vurdering av søk og navigasjon på skala,
    \item opplevde utfordringer, og
    \item forslag til forbedringer.
\end{itemize}

Undersøkelsen var anonym; det ble ikke samlet inn personidentifiserende opplysninger. Før distribusjon ble spørsmålsformuleringer, struktur og relevans kvalitetssikret i samarbeid med interaksjonsdesigner Christine Hørven i Helsedirektoratet, som arbeider med brukerundersøkelser for helsedirektoratet.no. Dette styrket undersøkelsens faglige forankring og sikret at spørsmålene var tilpasset kjente brukerbehov på nettstedet.

\subsubsection{Rekruttering og utvalg}
\label{sec:utvalg}

Målet med rekrutteringen var å nå et bredt spekter av brukere innenfor sentrale målgrupper for helsedirektoratet.no, herunder helsepersonell, ansatte i kommunal og statlig sektor, samt privatpersoner.

I tråd med survey-metodisk teori er sannsynlighetsutvalg (probability sampling) å foretrekke, da dette muliggjør generalisering fra utvalg til en definert populasjon \parencite[139-140]{creswell2014}. Et slikt design forutsetter både en klart avgrenset populasjon og en tilgjengelig \textit{sampling frame}, det vil si en fullstendig oversikt eller liste over alle i populasjonen som det kan trekkes tilfeldig fra. Videre må hver enhet ha kjent og lik sannsynlighet for å bli inkludert i utvalget.

I denne studien kunne målpopulasjonen overordnet defineres som brukere av helsedirektoratet.no. Nettstedet hadde imidlertid ikke en tilgjengelig brukerbase eller registrert oversikt over sine besøkende som kunne fungert som sampling frame for denne oppgaven. Det var derfor ikke mulig å etablere et sannsynlighetsutvalg.

Den opprinnelig planlagte distribusjonskanalen, \textit{Skyra.no}\footnote{\url{https://www.skyra.no/no}}, ville innebært at undersøkelsen ble vist til aktive brukere av helsedirektoratet.no. Selv om dette kunne gitt bredere kontekstuell dekning, ville deltakelse fortsatt vært basert på selvseleksjon blant besøkende, og dermed ikke utgjort et fullverdig sannsynlighetsutvalg. Distribusjonen gjennom skyra lot seg imidlertid ikke gjennomføre innenfor prosjektperioden, ettersom det allerede pågikk andre brukerundersøkelser på nettstedet.

Prosjektgruppen måtte derfor benytte alternative rekrutteringskanaler. Undersøkelsen ble distribuert gjennom følgende:

\begin{itemize}
    \item Interne kommunikasjonskanaler i Helsedirektoratet, formidlet via Christine Hørven.
    \item Direkte e-posthenvendelser til helseinstitusjoner og sykehus (resulterte i begrenset respons).
    \item Faglige og personlige nettverk med oppfordring til videre deling (Snøballrekruttering (snowball sampling)).
    \item Fysisk oppmøte ved Sykehuset i Gjøvik.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{Images/spørreundersøkelse/christine-undersøkelse}
    \caption{Eksempel på intern distribusjon av spørreundersøkelsen via Helsedirektoratets kommunikasjonskanaler.}
    \label{fig:christine-promotering}
\end{figure}

Figur~\ref{fig:christine-promotering} viser hvordan undersøkelsen ble formidlet internt, der ansatte i Helsedirektoratet ble oppfordret til å delta.

Rekrutteringsstrategien resulterte dermed i et ikke-sannsynlighetsutvalg, nærmere bestemt en kombinasjon av convenience sampling og snowball sampling. I slike utvalg har ikke alle medlemmer av populasjonen kjent eller lik sannsynlighet for å bli inkludert \parencite{creswell2014}. Metoden er hensiktsmessig for eksplorative studier og kontekstnær innsikt, men begrenser statistisk generaliserbarhet (se \autoref{sec:validitet}).
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.68\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Images/spørreundersøkelse/zoom.png}
        \caption{Plassering av en aktiv Skyra-undersøkelse på helsedirektoratet.no (06.02.2026).}
        \label{fig:skyra-undersokelse-kontekst}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.30\textwidth}
        \centering
        \includesvg[width=\textwidth]{Images/spørreundersøkelse/Skyra-undersøkelse}
        \caption{Spørreboksen brukt i undersøkelsen som danner grunnlag for sekundærdataene (15.–29. september 2025).}
        \label{fig:skyra-undersokelse-boks}
    \end{subfigure}

    \caption{Viser fram hvordan brukerundersøkelser levert gjennom Skyra presenteres på helsedirektoratet.no: (a) plassering på nettsiden når undersøkelsen er aktiv, og (b) selve spørreboksen slik den vises for besøkende.}
    \label{fig:skyra-undersokelse}
\end{figure}

Figur~\ref{fig:skyra-undersokelse} Illustrerer hvordan brukerundersøkelser levert gjennom Skyra typisk presenteres på nettstedet. Figuren er ment som en visuell kontekstualisering av distribusjonsmekanismen.


\subsubsection{Sekundærdata}
\label{sec:sekundaerdata}

Som supplement har prosjektgruppen hatt tilgang til eksisterende brukerinnsikt og tidligere undersøkelser fra Helsedirektoratet som var delt gjennom en powerpoint presentasjon og finnes i vedlegg \ref{app:Sekundærdata}. Disse benyttes for å vurdere om identifiserte utfordringer er vedvarende, og for å triangulere funn fra primærkartleggingen.
...
\paragraph{Kvantitativ analyse.}
\label{sec:kvant}
Lukkede spørsmål (skala og flervalg) ble analysert deskriptivt ved hjelp av frekvenser, fordelinger og sentralmål (gjennomsnitt/median der det var hensiktsmessig). Formålet var å identifisere mønstre i opplevd kvalitet på søk og navigasjon, og avdekke hvilke utfordringer som forekommer hyppigst.

\paragraph{Kvalitativ analyse.}
\label{sec:kval}
Åpne svar ble analysert tematisk ved å kode gjentakende mønstre i opplevde utfordringer, behov og forbedringsforslag. Kodene ble gruppert til overordnede tema som brukes som designgrunnlag for prototypen og som støtte i diskusjonen.
...
Etiske hensyn er særlig relevante gitt at løsningen omhandler helseinformasjon og offentlig forvaltning. Studien har ivaretatt personvern ved anonym datainnsamling, og prosjektet legger til grunn et prinsipp om at KI-støtte skal være forankret i verifiserbare kilder for å redusere risiko for feil eller misledende informasjon. Videre vektlegges transparens i presentasjon av resultater, slik at brukeren kan forstå og etterprøve treffene, og slik at potensielle skjevheter kan identifiseres og diskuteres.
