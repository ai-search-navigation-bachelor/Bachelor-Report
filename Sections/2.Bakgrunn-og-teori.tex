...
\label{sec:multilingual-e5}

\texttt{intfloat/multilingual-e5-base} \cite{wang2024multilingual} er en
bienkoder-modell (\textit{bi-encoder}) basert på XLM-RoBERTa, trent med
kontrastiv læring på flerspråklige spørring--passasje-par fra over 100 språk.
Modellen produserer 768-dimensjonale embeddings som plasserer semantisk
beslektede tekster nært hverandre i vektorrommet, uavhengig av eksakte
ordvalg og formuleringer.

E5-modellen bruker asymmetrisk enkoding: spørringer prefigeres med
\texttt{"query:~"} og dokumenter med \texttt{"passage:~"} for at modellen skal
lære ulike representasjoner for søk og innhold. Denne asymmetrien er vist å
gi bedre retrieval-ytelse enn symmetrisk enkoding
\cite{wang2024multilingual}.

Embedding-vektorer L2-normaliseres til enhetsvektorer, slik at kosinuslikhet
mellom en spørring $\mathbf{q}$ og et dokument $\mathbf{d}$ reduseres til
et prikkprodukt:
\[
\operatorname{sim}(\mathbf{q}, \mathbf{d}) = \mathbf{q} \cdot \mathbf{d},
\quad \|\mathbf{q}\| = \|\mathbf{d}\| = 1.
\]

Den flerspråklige arkitekturen gjør at modellen håndterer norsk tekst uten
oversetting, og fanger konseptuell likhet på tvers av faglig og folkelig
språkbruk -- noe som er særlig verdifullt for helseinformasjonssøk der
fagtermer og hverdagsspråk ofte refererer til samme fenomen.
...
% ─────────────────────────────────────────────────────────────
\subsection{Universell utforming og WCAG}
\label{sec:wcag-teori}

\textit{Web Content Accessibility Guidelines} (WCAG) er den internasjonalt
anerkjente standarden for tilgjengelig webinnhold, utgitt av World Wide Web
Consortium (W3C) \cite{wcag21}. Den gjeldende versjonen, WCAG~2.1, er bygget
rundt fire prinsipper -- Perceivable, Operable, Understandable og Robust
(POUR) -- og opererer med samsvarsnivåene A (minimum), AA og AAA. I Norge er
AA lovpålagt for offentlige virksomheter gjennom \textit{Forskrift om
universell utforming av IKT-løsninger} \cite{lovdata-uu}, hjemlet i
likestillings- og diskrimineringsloven~§~18. For navigasjonskomponenter er
de mest sentrale kriteriene 1.4.3 (fargekontrast), 2.1.1
(tastaturnavigasjon), 2.4.1 (hopplenke), 2.4.7 (fokusindikator) og
4.1.2 (navn og rolle på interaktive elementer).

Helsedirektoratets publiserte tilgjengelighetserklæring angir status
\textit{delvis samsvar} med WCAG~2.1 \cite{uustatus-helsedir}. Av
48 regulatoriske krav er 27 ikke oppfylt, og erklæringen identifiserer
spesifikt at noen knapper og lenker ikke er tilgjengelige via tastatur,
at fokusrekkefølgen er inkonsekvent på enkelte sider, og at interaktive
komponenter mangler programmatisk tilgjengelig navn og rolle (kriterium
4.1.2). Vår prototype for forbedret navigasjon og søk sikter mot å lukke
nettopp disse manglene.
