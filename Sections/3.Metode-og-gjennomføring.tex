...
Målet var å nå et bredt spekter av brukere fra sentrale målgrupper for helsedirektoratet.no: helsepersonell, kommunal/offentlig sektor og privatpersoner. Den opprinnelig planlagte distribusjonskanalen, \textit{Skyra.no}\footnote{\url{https://www.skyra.no/no}}, lot seg ikke gjennomføre innenfor prosjektperioden fordi det allerede pågikk andre brukerundersøkelser på nettstedet. Undersøkelsen ble i stedet distribuert gjennom følgende kanaler:

\begin{itemize}
    \item interne kommunikasjonskanaler i Helsedirektoratet, via Christine Hørven,
    \item direkte e-post til helseinstitusjoner og sykehus (gav svært få svar;
          flere institusjoner meldte at de ikke hadde anledning til intern distribusjon),
    \item faglige og personlige nettverk med oppfordring til videredeling
          (snowball-rekruttering), og
    \item et fysisk besøk ved Sykehuset i Gjøvik (bidro i begrenset grad til økt svarprosent).
\end{itemize}

Rekrutteringsstrategien resulterte i et \textit{ikke-sannsynlighetsutvalg} (convenience/snowball), noe som gir kontekstnær innsikt, men begrenser generaliserbarheten av funnene (se \autoref{sec:validitet}).

\subsubsection{Sekundærdata}
\label{sec:sekundaerdata}

Som supplement har prosjektgruppen hatt tilgang til eksisterende brukerinnsikt og tidligere undersøkelser fra Helsedirektoratet. Disse benyttes for å vurdere om identifiserte utfordringer er vedvarende, og for å triangulere funn fra primærkartleggingen.
