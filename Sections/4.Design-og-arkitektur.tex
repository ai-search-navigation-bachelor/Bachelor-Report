...
Frontend er bygget med React~\cite{reactjs-docs} og TypeScript og bruker Vite som
byggverktøy. Grensesnittet følger Digdirs designsystem og kommuniserer med backend via
et RESTful-API (Representational State Transfer).

Backend er implementert i Python med FastAPI-rammeverket~\cite{fastapi-docs}, et moderne
...
\begin{itemize}
    ...
    \item Backend tilbyr et proxy-endepunkt mot HAPI slik at API-nøkkelen ikke
...
Alle tjenester på VM-en kjøres som Docker-containere~\cite{docker-docs} og styres med
Docker~Compose. Konfigurasjonen i \texttt{docker-compose.yml} definerer to tjenester:

\begin{itemize}
    \item \textbf{db} -- MySQL~8-container med et vedvarende datavolum. Databasen
        initialiseres automatisk fra et SQL-oppsettskript ved første oppstart.
...
På VM-en fungerer Nginx~\cite{nginx-docs} som reverse proxy foran applikasjonen.
